...
export class FightService {
  ...
  async create(createFightInput: CreateFightInput): Promise<Fight> {
    const [fighter1, fighter2] = await this.fighterRepository.find({
      where: {
        id: In([createFightInput.fighter1Id, createFightInput.fighter2Id]),
      },
    });
    ...
    if (!event) {
      throw new NotFoundException(
        `No event found with id ${createFightInput.eventId}`,
      );
    }
    ...
    if (existingFights.length > 0) {
      throw new BadRequestException(
        `One of the fighters is already assigned to a fight in this event.`,
      );
    }
  ...
  async update(id: number, updateFightInput: UpdateFightInput): Promise<Fight> {
    ...
    await Promise.all([
      this.updateRanking(fighter1),
      this.updateRanking(fighter2),
    ]);
  ...
  async updateRanking(fighter: Fighter): Promise<void> {
    ...
    if (!ranking) {
      throw new NotFoundException(
        `Ranking not found for fighter ${fighter.name}`,
      );
    }
